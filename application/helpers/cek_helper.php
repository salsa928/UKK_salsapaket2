...
function cek_jika_user()
{
    ...
    $user = $check->db->get_where('tbl_masyarakat', ['nik' => $check->session->userdata('nik')])->row_array();
...
{
    $check = get_instance();
    $admin = $check->db->get_where('tbl_admin', ['username' => $check->session->userdata('username')])->row();
